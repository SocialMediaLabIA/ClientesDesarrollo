              ...
              return (
                ...
                >
                  {item.number < progress && (
                    <PiFlagPennantFill 
                    onClick={() => {pressProgressHandler(item.number)}}
                  ...
                  >
                    {openBar ? (
                      <h1 className="h-5 p-1 hover:mb-1">{item.title.toUpperCase()}</h1>
                    ) : (
                      <h1 className="h-5"></h1>
                    )}
                  </div>
                ...
                <React.Fragment key={`fragment-${index}`}>
                  {
                    ...
                    >
                      {item.number === progress && (
                        <p 
                        onClick={() => {pressProgressHandler(item.number)}}
                        className=" my-1 border-4 border-[#4de75a] bg-[#1bff1352] rounded-full w-6 h-6 p-2  flex justify-center items-center hover:scale-110 cursor-pointer"></p>
                      )}
                      {item.number < progress && (
                        <p 
                        onClick={() => {pressProgressHandler(item.number)}}
                        className="my-1 border-4 border-[#00DFFD] bg-[#00dffd5d] rounded-full w-6 h-6 p-2  flex justify-center items-center hover:scale-110"></p>
                      )}
                      {item.number > progress && (
                        <p 
                        onClick={() => {pressProgressHandler(item.number)}}
                        className="my-1 border-4 rounded-full w-6 h-6 flex justify-center items-center p-2 hover:scale-110"></p>
                      )}
